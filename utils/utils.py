...
def read_data(args):
    ...
    if not os.path.exists(f'data/{args.filename}_dtw_distance.npy'):
        ...
        np.save(f'data/{args.filename}_dtw_distance.npy', dtw_distance)

    dist_matrix = np.load(f'data/{args.filename}_dtw_distance.npy')
    ...
    if not os.path.exists(f'data/{args.filename}_spatial_distance.npy'):
        with open(filepath + file[1], 'r') as fp:
            ...
            np.save(f'data/{args.filename}_spatial_distance.npy', dist_matrix)
    ...
    dist_matrix = np.load(f'data/{args.filename}_spatial_distance.npy')
    # normalization
    std = np.std(dist_matrix[dist_matrix != np.float('inf')])
    mean = np.mean(dist_matrix[dist_matrix != np.float('inf')])
    dist_matrix = (dist_matrix - mean) / std
